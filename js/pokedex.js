...
const fetchPokemon = () => {
    ...
    fetch(url).then((res) => {
        /*** Manejo de errores. EL valor 200 significa que todo está OK***/
        if (res.status != "200") {
            changeImg("./assets/other/pokemontriste.png", "No existe ese pokemon :(");
            hideElements();
        } else {
            hideElements();
            return res.json();
...
const setType = (types) => {
    ...
    types.forEach(element => {
        ...
        let srcImgType = `./assets/types/${tipo}.png`;
